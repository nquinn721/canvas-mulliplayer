...
const GameComponent = observer(({ playerName, aiDifficulty, onReturnToHome }: GameComponentProps) => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      if (game.store?.socket) {
        game.store.socket.emit("changeAIDifficulty", { difficulty: aiDifficulty });
